...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule)
    {
        ...
        $schedule->command('loan:update')->cron('00 1 * * *')->appendOutputTo(public_path('/output.txt'));
        $schedule->command('deposit:accrue')->cron('00 1 * * *')->appendOutputTo(public_path('/output.txt'));
        $schedule->command('par:calculate')->cron('00 1 * * *');
        $schedule->command('holiday:implement')->everyMinute();

        ...
        <div class="col-lg-12">
            <div class="card">                
                <div class="card-header">   
                    ...
                    <div class="col-4">
                        <label for="" style="color:white" class="lead mr-2">Disbursement Date</label>
                        <input type="date" class="form-control" v-model="request.disbursement_date">
                    </div>
                ...
                <div class="card-body">
                    <table class="table table-striped">
                        ...
                        <tbody v-if="list.hasOwnProperty('data')">
                            <tr v-for="(item, key) in list.data.data" :key="key">
                                ...
                                <td><p class="title">{{item.client_fullname}}</p></td>
                                <td><p class="title">{{item.type}}</p></td>
                                <td><p class="title">{{moneyFormat(item.principal)}}</p></td>
                                <td><p class="title">{{moneyFormat(item.interest)}}</p></td>
                                <td><p class="title">{{moneyFormat(item.fees)}}</p></td>
                            </tr>
                            <tr>
                                <td></td>
                                <td></td>
                    ...
                    <paginator :dataset="list.data" @pageSelected="pageSelected" v-if="list.hasOwnProperty('data')"></paginator>
                </div>
            </div>
